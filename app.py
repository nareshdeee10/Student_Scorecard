...
<style>
    ...
    .topper-card {
        ...
        border-left: 8px solid #ff9800; box-shadow: 0 4px 10px rgba(0,0,0,0.1);
    }
    .rank-card {
...
with tab1:
    ...
    col2.metric("Class Average", f"{df['Percentage'].mean():.2f}%")
    col3.metric("Highest Percentage", f"{df['Percentage'].max():.2f}%")
    col4.metric("Pass Rate", f"{(df['Percentage'] >= 33).mean()*100:.1f}%")

    st.markdown("#### Top 10 Students by Percentage")
...
with tab2:
    ...
    # No .style.background_gradient() → safe for Streamlit Cloud
    ...
    st.markdown("### Congratulations to Our Toppers!")
...
# ==================== TAB 3 ====================
with tab3:
    st.markdown("### View Your Personal Rank Card")
    c1, c2 = st.columns(2)
    with c1:
    ...
    if st.button("Generate Rank Card", type="primary", use_container_width=True):
        if not roll or not auth:
            ...
            if student.empty:
                ...
                <div class="rank-card">
                    <h1>{s['Name']}</h1>
                    <p style="font-size:22px;">Class {s['Class']} | Roll No: {s['Roll No']}</p>
                    <h1 style="font-size:90px; color:#ffeb3b; text-shadow:4px 4px 12px black;">
                        Rank {int(s['Rank'])}
                    </h1>
                ...
                cols = st.columns(5)
                for col, sub in zip(cols, ["Maths","Science","English","Social","Hindi"]):
                    col.metric(sub, f"{s[sub]}/100")

                grade = {90:"A1", 80:"A2", 70:"B1", 60:"B2"}.get(next((k for k in [90,80,70,60] if s['Percentage'] >= k), 0))
                st.markdown(f"<h2 style='text-align:center; color:#4caf50;'>Overall Grade: {grade}</h2>", unsafe_allow_html=True)

# ==================== FOOTER ====================
st.markdown("---")
st.markdown("<p style='text-align:center; color:#888;'>© 2025 ABC International School • Made with ❤️ using Streamlit</p>", unsafe_allow_html=True)
